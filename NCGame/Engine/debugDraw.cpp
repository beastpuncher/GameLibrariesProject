...
{
	Renderer::Instance()->DebugDrawLine(v1 - Vector2D(size.x * 0.05f, 0.0f), v1 + Vector2D(size.x * 0.05f, 0.0f), color);
	Renderer::Instance()->DebugDrawLine(v1 - Vector2D(0.0f,size.y * 0.05f), v1 + Vector2D(0.0f,size.y * 0.05f), color);
}
